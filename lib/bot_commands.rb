...
module ::DiscordBot::BotCommands
  ...
  def self.manage_discord_commands(bot)
    ...
    # '!disckick' - a command to copy message history to Topics in Wilds City.
    ...
    # '!disckick' - a command to kick members beneath a certain trust level on Wilds City.
    ...
    bot.command(:disckick, min_args: 0, max_args: 1, bucket: :admin_tasks, rate_limit_message: I18n.t("discord_bot.commands.rate_limit_breached"), required_roles: [SiteSetting.discord_bot_admin_role_id], description: I18n.t("disckick.description")) do |event, min_trust_level|
      ...
      event.respond "Wilds City Kick:  Starting.  Minimum Trust Level = #{min_trust_level.to_s}"
      event.respond "Wilds City Kick:  Starting.  Please be patient, I'm rate limited to respect Discord services."
      event.respond "Wilds City Kick:  Preparing list of users who also have a registered account on Discord ..."
      ...
      event.respond "Wilds City Kick:  Determining user trust levels ..."
      ...
      event.respond "Wilds City Kick:  Compiling list of untrusted users ..."
      ...
      if can_do_the_magic_dance == true
        ...
        if ut_count > 0
          untrusted_users.each_with_index do |user, index|
            ...
            event.server.kick(user_id.to_s, "Kicked for not having sufficient trust level on the linked Wilds City site")
            event.respond "Wilds City Kick:  [#{index + 1}/#{ut_count}] <@#{user_id.to_s}> has been kicked for having insufficient trust level on the linked Wilds City site"
            sleep(SiteSetting.discord_bot_rate_limit_delay)
            rescue => e
              event.respond 'Wilds City Kick:  The user you are trying to kick has a role higher than/equal to me.'
          ...
          event.respond 'Wilds City Kick:  Great news!  There were no users below the specified or default trust level!'
        end
      else
        event.respond 'Wilds City Kick:  Sorry, but I do not have the "Kick Members" permission'
      end

      event.respond "Wilds City Kick:  I'm done with the dirty work!"
    end
    ...
    bot.command(:discsync, min_args: 0, max_args: 3, bucket: :admin_tasks, rate_limit_message: I18n.t("discord_bot.commands.rate_limit_breached"), required_roles: [SiteSetting.discord_bot_admin_role_id], description: 'Block users whose trust level is below a certain integer on Wilds City) do |event, clean_house, max_group_visibility, include_automated_groups|
      ...
      event.respond "Wilds City Sync:  Starting.  Please be patient, I'm rate limited to respect Discord services."
      event.respond "Wilds City Sync:  Checking if there are any eligible groups for sync ..."
      ...
      event.respond "Wilds City Sync: #{eligiblegroupbuilder.query.count} eligible group(s) were found"
      ...
      unless eligiblegroupbuilder.query.count == 0
        ...
        event.respond "Wilds City Sync:  Preparing list of users who also have a registered account on Discord ..."
        ...
        event.respond "Wilds City Sync:  Preparing list of groups that users who have a registered account on Discord belong to on Wilds City ..."
        ...
        event.respond "Wilds City Sync: #{discourse_groups.length} eligible group(s) were found with Discord users"

        unless discourse_groups.length == 0

          event.respond "Wilds City Sync:  Retrieving list of roles from Discord server ..."
          ...
          if clean_house.to_s.downcase == "true"
            ...
            event.respond "Wilds City Sync:  Deleting existing mapping roles ..."

            discourse_groups_count = discourse_groups.count

            discourse_groups.each_with_index do |g, index|

              event.respond "Wilds City Sync:  [#{index + 1}/#{discourse_groups_count}] Attempting to delete Role"
              ...
              unless role_id.nil?
                begin
                  event.server.role(role_id).delete("Wilds City Sync Cleanup")
                  event.respond "Wilds City Sync:  Role '#{g[:discourse_name]}' deleted as part of cleanup"
                  sleep(SiteSetting.discord_bot_rate_limit_delay)
                rescue => e
                  event.respond 'Wilds City Sync:  I dont appear to have rights to do this though!'
          ...
          event.respond "Wilds City Sync:  Creating missing Roles on Discord server ..."
          ...
          discourse_groups.each_with_index do |g, index|
            ...
            event.respond "Wilds City Sync:  [#{index + 1}/#{discourse_groups_count}] Attempting to create Role for #{g[:discourse_name]}"

            if !discord_roles.any? { |hash| hash[:name] == g[:discourse_name] }
              begin
                event.server.create_role(name: g[:discourse_name])
                event.respond "Wilds City Sync:  Role '#{g[:discourse_name]}' created!"
              rescue => e
                event.respond 'Wilds City Sync:  I dont appear to have rights to create Roles!'
              ...
              event.respond "Wilds City Sync:  Role '#{g[:discourse_name]}' already exists!"
            end
          ...
          event.respond "Wilds City Sync:  Building user role mapping ..."
          ...
          event.respond "Wilds City Sync:  Adding users to roles ..."

          ug_count = ug_list.count
          ug_list.each_with_index do |ug, index|
            event.respond "Wilds City Sync:  [#{index + 1}/#{ug_count}] Adding member '#{ug[:discourse_username]}' to '#{ug[:discourse_group_name]}'"
          ...
          event.respond "Wilds City Sync:  DONE!"
        else
          event.respond "Wilds City Sync:  No users were found in elibigle groups for sync using provided or default criteria!"
        end
      else
        event.respond "Wilds City Sync:  No eligible groups for sync using provided or default criteria!"
      end
    end

    bot.message(with_text: 'Ping!') do |event|
      event.respond 'Wilds City Pong!'
    end
